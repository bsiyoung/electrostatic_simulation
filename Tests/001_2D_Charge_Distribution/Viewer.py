...
class Viewer:
    ...
    def __init_default_configs(self):
        ...
        self.max_refresh_rate = int(1000 / 30)  # ms
    ...
    def draw_loop(self):
        ...
        if (len(self.draw_mgr.draw_queue) > 1000 or len(self.calc_mgr.calc_queue) == 0) and self.calc_mgr.idx_threshold > 5:
            self.calc_mgr.idx_threshold -= 3
        elif len(self.draw_mgr.draw_queue) < 600 and self.calc_mgr.idx_threshold < 200:
            self.calc_mgr.idx_threshold += 1

        '''
        print(len(self.calc_mgr.calc_queue),
              len(self.draw_mgr.draw_queue),
              self.calc_mgr.idx_threshold)
        '''
