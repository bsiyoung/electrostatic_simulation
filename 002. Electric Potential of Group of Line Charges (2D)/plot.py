...
class PlotWindow(QWidget):
    def __init__(self, data, lines, plot_size, window_size, grid_size, img_file):
        super().__init__()
    ...
    def draw_distribution(self, qp):
        ...
        for line in list(self.lines.keys()):
            x0 = int(half_window + pixel_per_meter * self.lines[line]['from']['x'])
            y0 = int(half_window - pixel_per_meter * self.lines[line]['from']['y'])
            x1 = int(half_window + pixel_per_meter * self.lines[line]['to']['x'])
            y1 = int(half_window - pixel_per_meter * self.lines[line]['to']['y'])
...
def draw_plot(data, lines, plot_size, window_size, grid_size, img_file):
    app = QApplication(sys.argv)
    ex = PlotWindow(data, lines, plot_size, window_size, grid_size, img_file)
    sys.exit(app.exec_())
